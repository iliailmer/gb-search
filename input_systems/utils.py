...
def get_f633():
    s = """2*u6 + 2*u5 + 2*u4 + 2*u3 + 2*u2 + 1, 2*U6 + 2*U5 + 2*U4 + 2*U3 + 2*U2 + 1, 1-4*u2*U3-4*u2*U4-4*u2*U5-4*u2*U6+4*u3*U2-4*u3*U4-4*u3*U5-4*u3*U6+4*u4*U2+4*u4*U3-4*u4*U5-4*u4*U6+4*u5*U2+4*u5*U3+4*u5*U4-4*u5*U6+4*u6*U2+4*u6*U3+4*u6*U4+4*u6*U5+2*u2+2*u3+2*u4+2*u5+2*u6,1-4*U2*u3-4*U2*u4-4*U2*u5-4*U2*u6+4*U3*u2-4*U3*u4-4*U3*u5-4*U3*u6+4*U4*u2+4*U4*u3-4*U4*u5-4*U4*u6+4*U5*u2+4*U5*u3+4*U5*u4-4*U5*u6+4*U6*u2+4*U6*u3+4*U6*u4+4*U6*u5+2*U2+2*U3+2*U4+2*U5+2*U6,
   U2*u2 -1,
   U3*u3 -1,
   U4*u4 -1,
   U5*u5 -1,
   U6*u6 -1""".split(
    ...
    var = list(parse_expr("U6,U5,U4,U3,U2,u6,u5,u4,u3,u2"))
    system = [parse_expr(x) for x in s]
    return (var, system)
...
def get_alea6():
    ...
    var = list(parse_expr("x0,x1,x2,x3,x4,x5"))
    s = """5*x0**2*x3+37*x1*x3*x4+32*x1*x3*x5+21*x3*x5+55*x4*x5,
    ...
    48*x0*x2*x3+2*x2**2*x3+59*x2**2*x5+17*x2+36*x3**3+45*x4""".split(
        ","
    )
    system = [parse_expr(x) for x in s]
    return (var, system)
...
def get_bayes148():
    ...
    s = """-x23*x32+x24*x31, -x22*x32+x24*x30, -x22*x31+x23*x30,
    -x21*x32+x24*x29, -x21*x31+x23*x29, -x21*x30+x22*x29, -x12*x32+x16*x28,
    -x19*x28+x20*x27,
    -x11*x31+x15*x27, -x18*x28+x20*x26, -x18*x27+x19*x26, -x10*x30+x14*x26,
    -x17*x28+x20*x25, -x17*x27+x19*x25, -x17*x26+x18*x25,
    -x9*x29+x13*x25, x20*x8-x24*x4,
    ...
    x19*x7-x23*x3, x18*x6-x2*x22, -x1*x21+x17*x5""".split(
        ","
    )
    system = [parse_expr(x) for x in s]
    return (var, system)
...
def get_cyclic7() -> List[str]:
    # cyclic7
    # all-around challenge
    system = [
        x.split(":=")[1].strip(" []:").replace("\n", "")
        for x in """var := [x0,x1,x2,x3,x4,x5,x6]:
...
def cyclic8():
    # cyclic8
    # all-around challenge
    return """var := [x0,x1,x2,x3,x4,x5,x6,x7]:
...
def cyclic9():
    # cyclic9
    # all-around challenge
    return """var := [x0,x1,x2,x3,x4,x5,x6,x7,x8]:
...
def get_eco12():
    ...
    return """var := [x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12]:
    sys := [
...
def get_gametwo7():
    ...
    return """var := [p1,p2,p3,p4,p5,p6,p7]:
    sys := [
...
def get_jason210():
    # jason210
    # sparse with many monomial divisions
    return """var := [x1, x2, x3, x4, x5, x6, x7, x8]:
...
def get_katsura7():
    system = [
        x.split(":=")[1].strip(" []:").replace("\n", "")
        for x in """var := [x0, x1, x2, x3, x4, x5, x6, x7]:
    ...
    2*x7+2*x6+2*x5+2*x4+2*x3+2*x2+2*x1+x0-1]:""".split(
            ":\n"
        )
    ]
    return system
...
def get_katsura8():
    ...
    system = [
        x.split(":=")[1].strip(" []:").replace("\n", "")
        for x in """var := [x0, x1, x2, x3, x4, x5, x6, x7, x8]:
    ...
    2*x8+2*x7+2*x6+2*x5+2*x4+2*x3+2*x2+2*x1+x0-1]:""".split(
            ":\n"
        )
    ]
    return system
...
def katsura9():
    ...
    return """var := [x0, x1, x2, x3, x4, x5, x6, x7, x8, x9]:
...
def get_katsura10():
    # katsura10
    # dense with many zero-reductions
    return """var := [x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10]:
...
def get_example_1():
    system = [
        parse_expr("x1*x5 + x5**2*(x6 + x7) + x7**2"),
        parse_expr("-x3 + x7 - (x2 - x5)*(x3 + x7)"),
        parse_expr("-2*x1 + x3 + x5 + 2*x6"),
        parse_expr("-x2 + x7"),
        parse_expr("x1 + x2 + x5 + x7"),
        parse_expr("-x1*x8*(x1 - x8) + x3 - x5"),
        parse_expr("x1 + x2 - x4*x8 - x4 - 2*x7 + x8"),
        parse_expr("x5*(x4*(x2 + x5) + x6)"),
    ]
    variables = symbols("x1:8")
...
def get_reimer8():
    var = parse_expr("x1,x2,x3,x4,x5,x6,x7,x8")
    s = """2*x1**2 - 2*x2**2 + 2*x3**2 - 2*x4**2 + 2*x5**2 - 2*x6**2 + 2*x7**2 - 2*x8**2 - 1,
    ...
    2*x1**9 - 2*x2**9 + 2*x3**9 - 2*x4**9 + 2*x5**9 - 2*x6**9 + 2*x7**9 - 2*x8**9 - 1""".split(
        ","
    )
    system = [parse_expr(x) for x in s]
    return (var, system)
